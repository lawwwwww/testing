...
<html>
	<head>
		...
		<title>View and Remove holiday</title>
...
<?php
	...
	if(isset($_POST['holiyear'])){
		...
		while($endtrigger ==0 ){
			...
			if($nextline==7){
				if($ggg->num_rows>0){
					echo "<td class='redtd'>".$day."<button id='$ddd' onclick='daded($ddd);'> x </button></td></tr>";
				}else{
					echo "<td class='bluetd'>".$day."</td></tr>";
				}
				$nextline = 0;
			}else{
				if($ggg->num_rows>0){
					
					echo "<td class='redtd'>".$day."<button id='$ddd' onclick='daded($ddd);'> x </button></td>";
				}else{
					echo "<td class='bluetd'>".$day."</td>";
				}
			}
...
?>
<p id = 'aaa'></p>
<form action="savetoholiminus.php" method="POST">
<!-- class="ng-hide" -->
<div class="ng-hide"><input type="text" name="holidayss" id="holid"></div>
<input type='submit' class="bth btn-primary" value='Remove Holiday'>
</form>
</div>
</body>
</html>
