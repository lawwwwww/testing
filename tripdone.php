...
<?php
		...
		if(isset($_GET["action"]))
	{
		if($_GET["action"]=="delete")
		{
			...
			$del="DELETE FROM tripdonetable where tripID=$_GET[id]";
...
<div class="container" data-ng-init="updatetripdonedata()">
...
<form>	
	...
	<br/>
		...
		<label for="search">Select the variable that you want to search for</label><br/>
         ...
         <!--allow the user to input their code and store it in obj.code-->
						...
						<br/><input type="checkbox" id="p_receive" name="p_receive" value="p_receive" data-ng-model="searchreceiver" data-ng-init="searchreceiver=false">
						<label for="p_receive">Receiver</label>
						<div data-ng-if="!searchreceiver">
							<div data-ng-init="obj.p_receive=''"></div>
						</div>
							<span data-ng-show="searchreceiver">
	...
	<table id="myTable2"  class="table table-responsive table-bordered table-striped table-hover">
	   <caption class="text-primary">Trip done</caption>
...
<tbody>
	<div class="row">
		<div class="col-md-12">
		<tr data-ng-repeat="u in filtered=(tripdonedata | orderBy:propertyName:reverse | filter:obj )| off: currentPage*studentsPerPage | limitTo : studentsPerPage"> 
	...
	<span data-ng-if="u.customer_sign!=NULL">
		...
		<td><a href="tripdone.php?action=delete&id={{u.tripID}}" onclick="remove()">
