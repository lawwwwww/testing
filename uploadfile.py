...
for qw in range(len(cust)):      #INSERT ALL THE EXTRACTED DATA INTO DATABASE
	mycursor.execute("INSERT INTO triptable(cust_name,areaID,collection_type,ship_date,driverID,truckID,adminID,salesOrderNo,address,container_size,trip_status,assign_status) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ",[cust[qw],area[qw],ctype[qw],fdate[qw],did[qw],truck[qw],sys.argv[2],salesOrder[qw],loc[qw],csize[qw],'Assign','Pending'])
	
	mycursor.execute("INSERT INTO uploadedtable(cust_name,areaID,collection_type,ship_date,driverID,truckID,adminID,salesOrderNo,address,container_size,trip_status,assign_status) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) ",[cust[qw],area[qw],ctype[qw],fdate[qw],did[qw],truck[qw],sys.argv[2],salesOrder[qw],loc[qw],csize[qw],'Assign','Pending'])
